...
int op_bit_get(const unsigned char* data, int i){
	...
	/*int bit = (data[i/8]>>(7-i%8))&0x1;
	printf("length: %lu\n",sizeof(data));
	printf("value of i: %d | value of i/8: %d\n",i,i/8);
	printf("data[%lu]: %02x\n",sizeof(data)-(i/8)-1,data[sizeof(data)-(i/8)-1]);
	printf("get bit position %d data[%lu]: %d\n",i%8,sizeof(data)-(i/8)-1,bit);
	printf("==================================\n");*/
