...
Nation *add_nation(Nation *db, unsigned int size, const char *p_nation)
{
    ...
    //The nation name is limited to 20 characters if the user
    //input a longer string the name will be trimmed to 20 characters
...
void update_medals(Nation* p_array, char* searchkey, int len, long *new_medals)
{
    for(int i=0;i<len;i++)
    {
        ...
        {
          p_array[i].gold+=new_medals[0];
          p_array[i].silver+=new_medals[1];
          p_array[i].bronze+=new_medals[2];
        }
    }
}
...
void save_table(Nation *p_array,char *file,int len)
{
    ...
    for ( int i = 0; i < len; i++) 
    {
        //Using ';' as the separator for each field of the data base
...
int main(void)
{
    ...
    printf("******************************************************\n");
    printf("Use the following syntax to create/add countries:\n");
    printf("- Add nation example: 'A Some_Country' \n");
    printf("- Modify nation example: 'M Some_Country 1 2 3' \n");
    printf("(1: # gold medals, 2: # silver and 3: # bronze medals)\n");
    printf("- 'L': Prints the current country database\n");
    printf("- Save database example: 'W file_name.txt' \n");
    printf("- Open database example: 'O file_name.txt' \n");
    printf("- Exit: 'Q' \n");
    printf("******************************************************\n");
    ...
    while(1)
    {
        ...
        if(str[0]=='M')
        {
            ...
            free(nation);
            //Re-initializing the variable medals for future use
            medals[0]=0;
            medals[1]=0;
            medals[2]=0;
            continue;
        ...
        if(str[0]=='L') 
        {
            //Sorting the data base before printing to the screen
